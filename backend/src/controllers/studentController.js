...
class StudentController {
  ...
  static async updateStudent(req, res) {
    try {
      ...
      try {
        ...
        if (phoneNumber || classId || dateOfBirth) {
          const updateStudentQuery = `
            UPDATE Students 
            SET ${phoneNumber ? 'phone_number = ?' : ''}
                ${phoneNumber && (classId || dateOfBirth) ? ',' : ''}
                ${classId ? 'class_id = ?' : ''}
                ${(classId && dateOfBirth) || (!classId && dateOfBirth) ? ',' : ''}
  ...
  static async deleteStudent(req, res) {
    try {
      ...
      await DatabaseService.execute('START TRANSACTION');
      try {
        // Với CASCADE DELETE, chỉ cần xóa User là tự động xóa Student + Exams
  ...
  static async bulkDeleteStudents(req, res) {
    try {
      ...
      await DatabaseService.execute('START TRANSACTION');
      try {
        // Với CASCADE DELETE, chỉ cần xóa Users là tự động xóa Students + Exams
